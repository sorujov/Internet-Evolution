...
```{r cellular-trend-plot, fig.cap="\\label{fig:cellular-trend-plot}Azərbaycan mobil internet sürətinin dinamikası"}
...
if (is_word) {
  ...
  ft5 <- set_header_labels(ft5,
                           ...
                           RSquared = "R²")
  ft5 <- set_caption(ft5, "Mobil internet trend statistikası\\label{tab:cellular-statistics}")
  ft5 <- theme_vanilla(ft5)
  ft5 <- colformat_num(ft5, j = "MonthlyIncrease", digits = 3)
  ft5 <- colformat_num(ft5, j = "RSquared", digits = 3)
  ...
  kable(cellular_trend_stats,
        ...
        caption = "\\label{tab:cellular-statistics}Mobil internet trend statistikası",
        digits = c(0, 3, 3)) %>%
...
```{r fixed-vs-cellular, fig.cap="\\label{fig:fixed-vs-cellular}Sabit və mobil internet müqayisəsi"}
