...
author: "İnformasiya Texnologiyaları və Rabitə Agentliyi (ICTA)"
...
# Prefer user-level library to avoid locked system packages during render
...
# Set fig.align only for non-Word outputs (Word doesn't support it)
if (!is_word) {
  knitr::opts_chunk$set(fig.align = "default")
}

# Fix UTF-8 encoding for PDF plots
if (knitr::is_latex_output()) {
  pdf.options(encoding = "UTF-8")
  options(
    device = function(...) {
      cairo_pdf(..., family = "DejaVu Sans")
    }
  )
}
...
# Set ggplot2 theme with Unicode-safe font for PDF
if (knitr::is_latex_output()) {
  theme_set(theme_minimal(base_size = 12, base_family = "DejaVu Sans") +
    theme(
...
```{r conclusions, child='sections/06_conclusions.Rmd'}
```
...
© 2025 İnformasiya Texnologiyaları və Rabitə Agentliyi. Bütün hüquqlar qorunur.
